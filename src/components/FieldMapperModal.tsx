...
export const FieldMapperModal: React.FC<FieldMapperModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-neutral-50 border border-neutral-200 rounded-2xl max-w-xl w-full p-6 shadow-2xl text-neutral-900">
        <div className="flex items-center justify-between pb-4 border-b border-neutral-200 mb-5">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-base font-bold text-neutral-900">網頁表單欄位名稱對應 (Field Mapping)</h3>
              <p className="text-xs text-neutral-500">若奇美網頁改版變更 HTML input name 屬性，可於此彈性修正</p>
            </div>
          </div>
          <button onClick={onClose} className="text-neutral-500 hover:text-neutral-900 px-2 py-1 text-xs rounded-lg border border-neutral-300">
            ✕
          </button>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-xs">
          <div>
            <label className="block text-neutral-700 font-semibold mb-1">加班日期欄位 (Date)</label>
            <input
              type="text"
              value={config.fieldMapping.dateField}
              onChange={(e) => handleFieldChange('dateField', e.target.value)}
              className="w-full bg-white border border-neutral-300 rounded-lg px-3 py-1.5 text-neutral-900 font-mono"
            />
          </div>

          <div>
            <label className="block text-neutral-700 font-semibold mb-1">起始時間 (Start Time)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-neutral-700 font-semibold mb-1">結束時間 (End Time)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-neutral-700 font-semibold mb-1">加班事由 (Reason)</label>
            <input
              type="text"
        ...
        <div className="mt-6 pt-4 border-t border-neutral-200 flex items-center justify-between">
          <button
            ...
            className="px-3 py-1.5 bg-neutral-100 hover:bg-neutral-200 text-neutral-700 text-xs rounded-lg border border-neutral-300 flex items-center gap-1"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 bg-blue-600 hover:bg-blue-600 text-white font-bold text-xs rounded-xl flex items-center gap-1.5"
          >
